...
; Written by Magnus Wedmark, 2013-2015
...
FxFam:
	...
	cmpi RC, 0xFF
	jz   opSetHigh
	jmp FinishOpcode ; This handles ALL 0xxx opcodes, which for org. platform was JMP 0xx 
Error1:
	jmp Error1 ; invalid opcode found
opCLS:
	;call FillScreenWith2
...
;Bit[7] - B
opSkpr:
	jmp FinishOpcode ; test debugging  
	push R0
	push R1
	push R2
	mov  R1, RA, ;or E8
...
SetPixelInBuffer: ; R0=Chip8X R1=Chip8Y R2=0/1 (OFF/ON)
	...
	mov  R5, R0
	shr  R5, 0x3 ; 2^3 => 8 pixels/byte
	mov  R6, R1
	shl  R6, 0x4 ; 2^3 => 8 bytes/line 3 for Chip8
	add  R6, R5 ; R6 = address
	ldi  R0, GraphicsBuffer
	add  R0, R6 	; add correct offset in graphics buffer to pixel
	call ReadByte 	; old value of R1 is destroyed, R1=byteval
	mov  R7, R1 	; save read value for pixel drawing
	;and  R7, R4 	; should and really be done?
	mov  R8, R2 	; copy pixelval to R8
	shl  R8, 7		; R8: 0x1=>0x80 and 0x0=>0x00
	shr  R8, R3		; R8 = Shifted bit to correct location
	xor  R7, R8 	; R7 = XOR for old and new pixel at correct bitposition
	mov  R2, R7
	mov  R1, R7
	call WriteByte
	pop  R1
	pop  R0
	; R2 should before call below hold the actual value that will be shown
	; Thus NOT the original R2 value from param, but the XOR'd value that will
	; be drawn. As long as 0=>0, any other value will passed as 1.
	and R2, R4
	; implement full PixDraw with XOR for old<>new pixel values
	ldm  R7, GraphicsMode ; Try to add this to get real Superchip pixels
	cmpi R7, 0x00
	jz	 ch8
	call SetSuperChipPixel
	jp   EndPixelSet2 
...
ClearBuffer:
	...
	ldi  R0, GraphicsBuffer
	;addi R0, 254
	addi R0, 1022 ; bigger buffer for SuperChip
	ldi  R1, 0
ClearBufferLoop:
	stm  R1, R0
	subi R0, 2
...
;SuperChipFont:
importbin superchip.font 0 160 SuperChipFont
SCFontSize equ 160
...
importbin C:\Projects\nChip16\Chip16Programs\COAC\SuperChipTest 0 22 Chip8Program
;importbin C:\Projects\nChip16\Chip16Programs\COAC\c8pixeltest.c8 0 34 Chip8Program
;importbin C:\Share\Emulators\Chip8\Tests\sctest_12\SCTEST 0 672 Chip8Program
;importbin C:\Share\Emulators\Chip8\Games\BLINKY 0 2356 Chip8Program
;importbin C:\Share\Emulators\Chip8\Games\TETRIS 0 494 Chip8Program
;importbin C:\Share\Emulators\Chip8\Games\15PUZZLE 0 264 Chip8Program
;importbin C:\Share\Emulators\Chip8\Games\HIDDEN 0 850 Chip8Program
;importbin C:\Share\Emulators\Chip8\Games\BRIX 0 280 Chip8Program
;importbin C:\Share\Emulators\Chip8\Games\GAMES\GAMES\TEST\IBM.ch8 0 132 Chip8Program 
;importbin C:\Share\Emulators\Chip8\Tests\KeypadTest\KeypadTest.c8 0 114 Chip8Program
;importbin C:\Share\Emulators\Chip8\FNC\RushHour.c8 0 3582 Chip8Program
;importbin C:\Share\Emulators\Chip8\FNC\Sokoban.c8 0 3583 Chip8Program
Chip8PrgSize equ 22

;include GraphicsBuffer.asm
